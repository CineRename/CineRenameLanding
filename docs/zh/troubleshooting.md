...
2. 检查**标题 + 年份**是否能被 TheTVDB 或 TMDB 识别。如果不能，请显式添加 ID（例如 `{tmdb-12345}`）。
3. 执行**全面扫描**并强制更新元数据。
4. 如果以上都不起作用，请将文件移出媒体库，扫描，然后将文件移回，再次扫描 (Plex Dance)。

## 字幕未下载

- 检查您的 **OpenSubtitles API 密钥**（设置 → Providers）。
...
- 检查**日志**（`设置 → 高级 → 详细程度 = debug`）以识别出速度慢的步骤。
...
**设置 → 高级 → 详细程度 (Verbosity)**：

- `error` — 仅限错误（默认）
- `warn` — 错误 + 警告
- `info` — 关键事件
- `debug` — 对诊断有用的详细信息
- `trace` — 极其繁杂的输出，用于开发调试

或者通过环境变量 `CINERENAME_LOG_LEVEL=debug`。
